...
```{r edav.lab}
sirfunctions::edav_io("read", file_loc = get_constant("CLEANED_LAB_DATA")) 
...
As of late 2026, lab data have been pre-cleaned before upload to EDAV/sharing with analysis partners. If any lab data quality issues are identified during this step, please contact CDC so the cleaning process can be updated. If you only have the raw Excel files from WHO, run prep_lab_data() from tidypolis.
...
lab_data <- clean_lab_data(lab_data, start_date, end_date, ctry.data$afp.all.2, Sys.getenv("DR_COUNTRY"))
