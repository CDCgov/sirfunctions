...
```{r edav.lab}
ctry.data$lab.data <- sirfunctions::edav_io("read", file_loc = get_constant("CLEANED_LAB_DATA")) |> 
  filter(!is.na(EPID))
```
...
lab_data_errors(ctry.data$lab.data, ctry.data$afp.all.2)
...
ctry.data$lab.data <- clean_lab_data(ctry.data$lab.data, start_date, end_date, 
...
```{r set.recent.shape}
ctry.shape <- load_clean_ctry_sp(ctry_name = Sys.getenv("DR_COUNTRY"), 
                                 type = "long"
                                 )
prov.shape <- load_clean_prov_sp(ctry_name = Sys.getenv("DR_COUNTRY"), 
                                 type = "long"
                                 )
dist.shape <- load_clean_dist_sp(ctry_name = Sys.getenv("DR_COUNTRY"), 
                                 type = "long"
                                 )
```
...
generate_afp_case_map(ctry.data$afp.all, ctry.shape, prov.shape, start_date) # issues here
