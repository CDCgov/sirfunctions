...
2.  **Set the desk review parameters**. `local_dr_path` is the folder where you would like to store desk reviews and associated files locally. If lab or ISS/eSurv data are unavailable locally, please assign the path values to `NULL`. Otherwise, assign the path to the local files for loading in R. For multiple countries, pass as a string vector, i.e. `c("djibouti", "somalia", "yemen")`.
...
```{r lab.timeliness}
lab.timeliness.ctry <- generate_lab_timeliness(lab_data |> dplyr::filter(ctry %in% Sys.getenv("DR_COUNTRY")), "ctry", start_date, end_date)
lab.timeliness.prov <- generate_lab_timeliness(lab_data |> dplyr::filter(ctry %in% Sys.getenv("DR_COUNTRY")), "prov", start_date, end_date)
```
...
inad.tab.flex <- generate_inad_tab(ctry.data$afp.all.2, cstool, start_date, end_date) 
...
#### Optional PROV analyses and PowerPoints
Set `run_prov_analysis` to `TRUE` to create PROV-level figures and one
PowerPoint for each PROV. By default, all available PROVs are used. You may
instead supply a character vector to `prov_names`. The PROV output includes
the AFP case map and epicurve, NPAFP and stool-adequacy maps and tables, the
60-day follow-up table, PROV population-and-roads map, district timeliness
panels, and environmental-surveillance outputs. The NPAFP immunization graph,
age histogram, and delayed-detection graph are intentionally not generated.

```{r optional.prov}
run_prov_analysis <- FALSE
prov_names <- NULL
# To analyze selected PROVs instead of all available PROVs, use for example:
# prov_names <- c("PROV 1", "PROV 2")
# For one PROV:
# prov_names <- "PROV 1"
if (run_prov_analysis) {
  if (!exists("generate_prov_desk_review_outputs")) {
    stop(
      "PROV functions are not loaded. Re-run init_dr() with source = TRUE, " ,
      "or use a sirfunctions version that includes PROV functions."
    )
  }
  if (is.null(prov_names)) {
    prov_names <- list_desk_review_prov(ctry.data)
  }
  prov_figure_dir <- file.path(Sys.getenv("DR_FIGURE_PATH"), "prov")
  prov_ppt_dir <- file.path(Sys.getenv("DR_POWERPOINT_PATH"), "prov")
  dir.create(prov_figure_dir, recursive = TRUE, showWarnings = FALSE)
  dir.create(prov_ppt_dir, recursive = TRUE, showWarnings = FALSE)
  prov_outputs <- stats::setNames(
    lapply(prov_names, function(prov_name) {
      tryCatch(
        generate_prov_desk_review_outputs(
          ...
          end_date = end_date,
          output_path = prov_figure_dir
        ),
        error = function(e) {
          cli::cli_alert_warning(
            paste0("Skipping ", prov_name, ": ", conditionMessage(e))
          )
          NULL
        }
      )
  ...
  failed_prov_names <- prov_names[
    vapply(prov_outputs, is.null, logical(1))
  ]
  prov_outputs <- prov_outputs[!vapply(prov_outputs, is.null, logical(1))]
  prov_names <- names(prov_outputs)
  if (length(failed_prov_names) > 0) {
    cli::cli_alert_warning(paste0(
      "No PowerPoints will be written for: ",
      paste(failed_prov_names, collapse = ", ")
    ))
  }
  prov_ppt_files <- stats::setNames(
    vapply(prov_names, function(prov_name) {
      tryCatch(
        write_prov_desk_review_ppt(
          prov_outputs = prov_outputs[[prov_name]],
          prov_name = prov_name,
          start_date = start_date,
          end_date = end_date,
          ppt_output_file = file.path(
            prov_ppt_dir,
            paste0(
              "Deskreview_",
              gsub("(^-+|-+$)", "", gsub("[^a-z0-9]+", "-", tolower(trimws(prov_name)))),
              ".pptx"
            )
          ),
        ...
        error = function(e) {
          cli::cli_alert_warning(
            paste0("PowerPoint failed for ", prov_name, ": ", conditionMessage(e))
          )
          NA_character_
        }
      )
