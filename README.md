...
Ensure that `pak` is installed (if not run `install.packages("pak")`). Then run the following command:
...
Please send a message to Stephanie Kovacs at
[uvx4\@cdc.gov](mailto:uvx4@cdc.gov) for any questions or
queries.
