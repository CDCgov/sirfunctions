...
Ensure that `devtools` is installed (if not run
`install.packages("devtools")`). Then run the following command:
...
are available to the users. Please refer to the manual for full function
specifications.
...
Please send a message to Nishant Kishore at <ynm2@cdc.gov> for any
