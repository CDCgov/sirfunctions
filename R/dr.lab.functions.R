...
lab_data_errors <- function(lab.data, afp.data,
  ...
  if ("EpidNumber" %in% lab.data.cols) {
...
clean_lab_data <- function(lab_data, start_date, end_date,
  ...
  if ("MasterKey" %in% lab_data_cols) {
    ...
    if ("prov" %in% lab_data_cols) {
      cli::cli_alert_warning("Lab data already cleaned. Filtering to country and adding rolling years.")
      ...
      cli::cli_alert_warning("Lab data not cleaned. Run tidypolis::prep_lab_data() function to clean and add locations.")
    }
  }
}
...
#'
#' @details this function is no longer referenced within the sirfunctions package.
#'
#' Cleans the lab data from WHO. This was previously used in [clean_lab_data()], but can be used on its own.
#'
#' @inheritParams clean_lab_data
#' @returns `tibble` Cleaned lab data.
#' @examples
...
#' `r lifecycle::badge("deprecated")`
#'
#' @details Cleans regional lab data. This was previously used in [clean_lab_data()], but can be used on its own.
#' @inheritParams clean_lab_data
#' @returns `tibble` Cleaned lab data.
#' @examples
#' \dontrun{
